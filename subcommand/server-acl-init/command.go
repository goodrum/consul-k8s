...
func (c *Command) init() {
	...
	c.flags.BoolVar(&c.flagCreateSnapshotAgentToken, "create-snapshot-agent-token", false,
		"Toggle for creating a token for the Consul snapshot agent deployment")
...
func (c *Command) createACL(name, rules string, consulClient *api.Client) error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Error creating %s policy: %s", name, err))
	}
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Error creating %s token: %s", name, err))
	}
